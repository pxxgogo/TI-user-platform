...
var gesture_description_list = ["五指张开，“擦玻璃”", "五指张开，手掌左右扇动", "五指张开，手掌前后反转", "手张开->握拳", "手握拳->张开",
    "用食指指点：从前方指向右前方，再回到前方", "用食指在空中点击", "用食指画圈", "写字", "字母 O 的手势 (O)", "竖起拇指，左右摇动 (10)", "数字 6 的手势 (Y)",
    "食指、中指交叉 (R)", "OK 的手势 (F)", "手腕略向下弯曲，拇指和食指指向下 (Q)"];
...
function get_random_int() {
    return Math.floor(Math.random() * (gesture_max_index + 1));
}
...
function update_gesture() {
    current_gesture_index = get_random_int();
